...
import { Taskes } from '../models/taskes';
...
tz:string;
  ...
  getAllMatchMaker(): Observable<User[]> {
    return this.http.get<User[]>(environment.api + '/getAllMatchMaker')
...
// מטלות
  GetAllTaskes():Observable<Taskes[]>{
    return this.http.get<Taskes[]>(environment.api+"/GetAllTaskes");
  }


  removeTask(t:Taskes){
    return this.http.post(environment.api+"/removeTask",t);
  }
  addCommentFotTask(t:Taskes){
    return this.http.post(environment.api+"/addCommentFotTask",t);
  }
}
